...
export const PhotoUploadModal = ({ isOpen, onClose, onUploadSuccess, trips = [] }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Upload Trip Photos" maxWidth="600px">
      <form onSubmit={handleUpload} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        <div className="form-group" style={{ marginBottom: 0 }}>
          ...
          <select value={tripId} onChange={(e) => setTripId(e.target.value)} className="form-input no-icon">
            <option value="">Unlinked Photo Upload</option>
            {trips.map(t => (
        ...
        <div className="form-group" style={{ marginBottom: 0 }}>
          ...
          <input
            ...
            placeholder="e.g. Sunset view from Ubud ridge walk"
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '0.5rem' }}>
          ...
          <button type="submit" disabled={uploading || !selectedFiles.length} className="btn btn-primary">
            {uploading ? 'Uploading Photos...' : `Upload ${selectedFiles.length} Photo(s)`}
          </button>
        </div>
      </form>
    </Modal>
